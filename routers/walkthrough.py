...
router = APIRouter(prefix="/api/walkthrough")
...
async def get_data_user(data_format="json"):
    filepath = "csvs/walkthrough/dummy-user-data.csv"
...
async def get_sensitive_user_data(data_format="json"):
    filepath = "csvs/walkthrough/dummy-sensitive-user-data.csv"
    # guard clause 1 (provided query parameter data_format=csv)
    if data_format == "csv":
...
async def get_transaction_data(data_format="json"):
    filepath = "csvs/walkthrough/dummy-transaction-data.csv"
    # guard clause 1 (provided query parameter data_format=csv)
    if data_format == "csv":
...
async def get_ip_data(data_format="json"):
    filepath = "csvs/walkthrough/dummy-ip-data.csv"
    # guard clause 1 (provided query parameter data_format=csv)
    if data_format == "csv":
